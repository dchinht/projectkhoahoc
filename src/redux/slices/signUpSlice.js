...
export const getListSignUp = createAsyncThunk("getListSignUp", async (newData) => {
    ...
    extraReducers : (builder) => {
       ...
       builder.addCase(login.fulfilled,(state,action)=>{
        ...
        let user = action.payload.users.find(
            (user) => user.username === action.payload.infoLogin.username 
            &&  user.password === action.payload.infoLogin.password 
        );
        // kiểm tra đăng nhậP
        if(!user) {
            notification("usernam or password sai mất rồi em ơi, thử lại đi nhé !",'error');
        }else{
